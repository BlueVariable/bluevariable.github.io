...
- `_tools/tear-masks.js` — generator for the torn-edge masks (see Conventions). Underscore-prefixed, so Jekyll does not publish it.
...
- Torn paper edges are generated, not images. `_tools/tear-masks.js` (plain Node, no dependencies, not published) prints the SVG data-URI masks (gradient + `feTurbulence` speckle, hard-thresholded) that live in the `--tear-*` custom properties in `site.css`; after changing its parameters, run `node _tools/tear-masks.js` and paste the output between the "generated by" markers. Sections with `.torn-bottom` are masked (solid area plus the `--tear-b` band, `mask-size: 100% <h>`, no repeat so there are no seams) and the next block is pulled up under them; `.torn-top` sections, the news bands, the header strip and the page-transition band use `--tear-t`; the Games panel adds `--tear-l`/`--tear-r`. Each direction has three frames (same speckle, different low-frequency displacement) and a `steps(1)` animation on `:root` cycles `--tear-*` through them for a subtle line boil; the reduced-motion rule stops it on frame 1.
